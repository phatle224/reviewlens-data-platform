...
| Trạng thái phase hiện tại | `IN_PROGRESS` — 6/15 M4 contract, DLP, catalog, selector and prompt items complete |
...
| M4 | `IN_PROGRESS` | Schema/taxonomy/version contract, DLP, catalog evidence, selector and untrusted-evidence prompt complete | [Overview](./phases/M4/README.md) · [Checklist](./phases/M4/M4_CHECKLIST.md) · [Tests](./phases/M4/M4_TEST_CASES.md) |
...
- Gate local sau M4 bundle: Ruff format/lint cho `src`/`tests`, strict mypy, dbt parse `--warn-error`, 518 offline tests (8 opt-in live skips, 86.12% coverage), artifact lock, repository policy và status validator pass. Full suite dùng workspace-local pytest temp do Windows user-temp bị access-denied.
...
| M4 | 12 `PASS`, 8 `PENDING`, 0 `FAIL`, 0 `DEFERRED` | [M4 test cases](./phases/M4/M4_TEST_CASES.md); catalog read, selector and prompt contracts add no paid/provider-content call |
| Quality | `PARTIAL` | Ruff, strict mypy, dbt parse, 518 offline tests (8 opt-in live skips, 86.12% coverage), artifact lock, repository policy and status validator pass. Dependency audit flags 12 known CVEs in Airflow 3.3.0/sqlparse 0.5.5; remediation is tracked before M8/container release. |
...
1. Implement `IMP-M4-007…009`: structured-output client, semantic validation, one repair path and bounded retry/quarantine/resume using fakes plus an opt-in synthetic live smoke only.
2. Implement hard budget controls (`IMP-M4-010`) before any bounded real-review pilot or applying migration `009` live.
