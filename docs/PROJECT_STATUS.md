...
| Trạng thái phase hiện tại | `IN_PROGRESS` — 14 done, 1 partial, 5 not started |
...
| M1 | `IN_PROGRESS` | Config, identities, provider/dbt/Airflow boundaries, audit/logging and authenticated local app shell | [Overview](./phases/M1/README.md) · [Checklist](./phases/M1/M1_CHECKLIST.md) · [Tests](./phases/M1/M1_TEST_CASES.md) |
...
- Hoàn tất `IMP-M1-012`: authenticated Streamlit foundation shell và canonical-config `reviewlens-app` launcher chỉ bind loopback.
- Masked token gate dùng fixed-length digest/constant-time comparison, xóa candidate sau attempt, chặn anonymous/wrong/missing/oversized token và hỗ trợ sign-out.
- Readiness chỉ hiển thị boolean configuration state `ready/degraded/unavailable`, không gọi provider; M1 page ghi rõ synthetic evidence và chưa có active analytics release.
- 15/15 focused AppTest/unit/security tests pass, gồm remote bind/auth-disable negative, error/leak canaries và launcher security flags.
- Không gọi Snowflake/R2/OpenRouter/Chroma, không đọc Olist row và không phát sinh project service cost.
- Full offline gate: 133 pass, 5 expected live skips, 91.06% coverage; Ruff + Airflow 3 rules, mypy, dbt warnings-as-errors, lock check và locked offline Airflow+dbt sync pass.
...
| M1 | 33 `PASS`, 8 `PENDING`, 0 `FAIL`, 0 `DEFERRED` | [M1 test cases](./phases/M1/M1_TEST_CASES.md); offline 133 pass/5 live skip; authenticated AppTest + logging/audit/Airflow/dbt/provider/R2/stage/RBAC/JWT evidence |
| Quality | `PASS` | Ruff format/lint + Airflow 3 rules, mypy strict, dbt warnings-as-errors, 91.06% branch-aware coverage, uv lock check và locked offline Airflow+dbt sync |
...
|---|---|---|
| OpenRouter | 5 USD/project; warning 0.50 USD/day | Không gọi trong phiên app shell; 0 USD phát sinh từ code path project |
| Snowflake | ≤10 credits/month; X-Small, auto-suspend 60s | App/readiness tests offline; dbt placeholder compile không kết nối provider; prior JWT evidence remain pass |
...
1. `IMP-M1-015`: CI quality/security/data-leak/dependency gates trên foundation contract hiện đã ổn định.
2. `IMP-M1-016`: non-root app/Airflow container images sau CI contract.
3. `IMP-M1-017`: single-local Compose và immutable artifact metadata sau container gate.
