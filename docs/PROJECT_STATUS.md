...
| Trạng thái phase hiện tại | `IN_PROGRESS` — 8/15 M4 items complete; structured provider smoke remains opt-in |
...
| M4 | `IN_PROGRESS` | DLP through retry/quarantine baseline complete; structured provider smoke remains opt-in | [Overview](./phases/M4/README.md) · [Checklist](./phases/M4/M4_CHECKLIST.md) · [Tests](./phases/M4/M4_TEST_CASES.md) |
...
- Gate local sau M4 bundle: Ruff format/lint cho `src`/`tests`, strict mypy, dbt parse `--warn-error`, 529 offline tests (9 opt-in live skips, 86.18% coverage), artifact lock, repository policy và status validator pass. Full suite dùng workspace-local pytest temp do Windows user-temp bị access-denied.
...
| M4 | 13 `PASS`, 7 `PENDING`, 0 `FAIL`, 0 `DEFERRED` | [M4 test cases](./phases/M4/M4_TEST_CASES.md); provider smoke is intentionally pending explicit cost opt-in |
| Quality | `PARTIAL` | Ruff, strict mypy, dbt parse, 529 offline tests (9 opt-in live skips, 86.18% coverage), artifact lock, repository policy and status validator pass. Dependency audit flags 12 known CVEs in Airflow 3.3.0/sqlparse 0.5.5; remediation is tracked before M8/container release. |
...
1. Implement `IMP-M4-010`: token/cost estimator, 0.50 USD warning and 5 USD hard stop; it must guard the live smoke and every future dispatch.
2. Implement committed result/coverage baseline (`IMP-M4-011`) only after budget controls; do not apply migration `009` live yet.
