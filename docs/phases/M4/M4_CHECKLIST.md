...
| IMP-M4-012 | `PARTIAL` | Create stratified golden/holdout and semantic evaluator | Deterministic private-label splitter stratifies score/aspect/length/opaque-category/delivery outcome, reserves ≥20% blind holdout, evaluates only declared holdout IDs, and reports macro sentiment/aspect F1, topic micro F1 and schema pass rate. Private-only tooling generated a deterministic 200-row annotation queue/template from the local archive, stratified by score/length/opaque-category/delivery before aspect is human-labelled. Owner-authorized local score/delivery suggestions now exist with `machine_assisted` status but are intentionally rejected by the human-golden loader; review/approval and real evaluation remain open. |
