...
| TC-M1-037 | Rotation live | Controlled named-key rotation/revocation smoke without admin fallback | New key succeeds with exact role; old key is denied after configured grace/revoke | `PENDING` | Isolated `ANALYST_ROLE` canary harness passes offline with two-layer opt-in, stale-key preflight and guaranteed cleanup; initial runtime key remains untouched. Awaiting explicit owner approval for the live authentication mutation |
...
- TC-M1-037 remains `PENDING`: no live flag was enabled, no provider call occurred, no key metadata changed and no project service cost was incurred. Owner confirmation is still required before the isolated live authentication mutation.
