...
| TC-M1-004 | Unit | Full pytest suite | Tất cả unit/contract tests pass | `PASS` | 151 passed, 5 expected live skips, 90.87% branch-aware coverage |
...
| TC-M1-023 | CI | Workflow required gates | Static workflow contract pass | `PASS` | `.github/workflows/ci.yml` least-privilege/pinned-action contract và deliberate failing policy fixture pass; workflow không dùng secret/live flag, không silence failure và giữ container gate fail-closed |
